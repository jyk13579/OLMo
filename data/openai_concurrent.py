...
def main():
    ...
    parser.add_argument("--requests-per-minute", type=int, default=60,
                        help="Number of requests per minute per API key")
    parser.add_argument("--expected_response_seconds", type=float,
                        default=5, help="Number of seconds to wait for a response")
    args = parser.parse_args()

    openai_concurrent = OpenAIChatCompletionConcurrent(
        api_keys=API_KEYS, requests_per_minute=args.requests_per_minute, expected_response_seconds=args.expected_response_seconds)
    openai_concurrent.create_many_file(
        input_path=args.input_path, output_path=args.output_path, fail_path=args.fail_path)
...
class OpenAIChatCompletionConcurrent:
    def __init__(self, api_keys: List[str], requests_per_minute: int = 60, expected_response_seconds: float = 5.0):
        ...
        requests_per_second = self.requests_per_minute / 60  # 60/60=1
        simultaneous_num_requests = requests_per_second * \
            self.expected_response_seconds  # 1*5=5
        buffer = 2
        self.num_workers = int(simultaneous_num_requests * buffer)  # 5*2=10

        total_requests_per_second = requests_per_second * self.num_api_keys  # 1*1=1
        self.time_between_requests = 1 / total_requests_per_second  # 1/1=1
    ...
    def create_many(self, requests: List[dict], streaming=False):
        ...
        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            for item_index, item in enumerate(requests):
                ...
                future = executor.submit(
                    call_and_return, api_key=api_key, item=item)
    ...
    def create_many_file(self, input_path: str, output_path: str, fail_path: str):
        ...
        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            for item_index, item in enumerate(requests):
                api_key = self.api_keys[item_index % self.num_api_keys]
                executor.submit(call_and_write, api_key=api_key, item=item,
                                output_path=output_path, fail_path=fail_path)
                sleep(self.time_between_requests)


def call_and_return(api_key: str, item: dict):
    try:
        # print(item["request"]["messages"][1]["content"])
